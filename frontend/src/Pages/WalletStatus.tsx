  ...
  useEffect(() => {
    const checkApproval = async () => {
      ...
      try {
        ...
        // If blockchain check returned false, check fallbacks
        if (!approved) {
          console.log('⚠️ Blockchain returned false, checking fallbacks...');
          
          // Check persisted approvals from localStorage
          const persistedApprovals = getPersistedApprovals();
          const persistedStatus = persistedApprovals[userAddress];
          
          if (persistedStatus === true) {
            console.log(`💾 Found persisted approval for ${userAddress}, using that`);
            setBlockchainApproved(true);
            return;
          }
          
          // Check user's IsApproved from localStorage (database status)
          if (user?.IsApproved === true) {
            console.log(`💾 User has IsApproved=true in database, using that`);
            // Also persist this for future use
            const newPersistedApprovals = getPersistedApprovals();
            newPersistedApprovals[userAddress] = true;
            setPersistedApprovals(newPersistedApprovals);
            setBlockchainApproved(true);
            return;
          }
          
          console.log(`ℹ️ No fallback approval found for ${userAddress}`);
        }
        ...
        // Even on error, check fallbacks
        const userAddress = user?.WalletAddress;
        if (userAddress) {
          const persistedApprovals = getPersistedApprovals();
          if (persistedApprovals[userAddress] === true) {
            console.log(`💾 Error occurred but found persisted approval for ${userAddress}`);
            setBlockchainApproved(true);
            return;
          }
          if (user?.IsApproved === true) {
            console.log(`💾 Error occurred but user has IsApproved=true`);
            setBlockchainApproved(true);
            return;
          }
        }
    ...
    checkApproval();
  }, [user?.WalletAddress, user?.IsApproved, isConnected, address, provider]); // Re-run when user data or wallet changes
