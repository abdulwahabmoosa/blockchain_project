  ...
  useEffect(() => {
    const fetchWalletBalance = async () => {
      ...
      if (isConnected && address && provider) {
        try {
          // Use connected wallet address (may be different from user's registered address)
          const balanceAddress = address.toLowerCase() === userAddress?.toLowerCase() ? userAddress : address;
          console.log(`💰 Fetching ETH balance for ${balanceAddress}`);
          ...
          console.log(
            `✅ MetaMask balance (on-chain): ${onChainBalanceEth} SepoliaETH, scaled: ${scaledBalance} ETH`
          );
